...
void mem_nop2(void* ptr, int size) // thats calling things you know xDDD
{
...
Hooking::NativeHandler  ORIG_NETWORK_SESSION_KICK_PLAYER = NULL; /// Can i check something for detecting? network // do you use sigs? idk to use this :( second
...
void Hooking::FindPatterns()
{
	...
	//auto p_modelCheck = pattern("48 85 C0 0F 84 ? ? ? ? 8B 48 50"); // This is the key thats why its doesn work ;) we replace thatt in CPattern second
	//auto p_modelSpawn = pattern("48 8B C8 FF 52 30 84 C0 74 05 48"); // This also
	...
	CPattern modelCheckPattern("\x48\x85\xC0\x0F\x84\x00\x00\x00\x00\x8B\x48\x50", //Thats the new modelcheck
		"xxxxx????xxx");

	CPattern modelSpawnPattern("\x48\x8B\xC8\xFF\x52\x30\x84\xC0\x74\x05\x48", // This is the new modelspawn // Please dont share it i mean its was ver expesive to upgrade it ;) i dont share bro that why i trust you and will help you 
		"xxxxxxxxxxx");
	...
	if (bypassPtr != nullptr) {
		...
		Log::Msg("Failed to find online model requests bypass");
	}
